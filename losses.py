...
class IBLoss(nn.Module):
    ...
    def forward(self, input, target, features):
        # soft_input = F.softmax(input, dim=1)
        # onehot_target = F.one_hot(target,args.num_classes)
        # abs = torch.abs(soft_input-onehot_target)
        # grads = torch.sum(abs, 1)
        grads = torch.sum(torch.abs(F.softmax(input, dim=1) - F.one_hot(target, args.num_classes)),1) # N * 1
