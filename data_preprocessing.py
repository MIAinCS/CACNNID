        ...
        for fcount, img_file in enumerate(tqdm(file_list_path)):
            ...
            if mini_df.shape[0] > 0:
                ...
                for node_idx, cur_row in mini_df.iterrows():
                    ...
                    node_cube.astype(np.float16)
                    # save as npy file
                    print(index)
                    if label == 1:
                        filepath = output_path + "1/" + "subset" + str(subsetindex) + "/"
                        if not os.path.exists(filepath):
                            os.makedirs(filepath)
                        filename = str(fcount) + "_" + str(index)
                        np.save(filepath + filename + ".npy", node_cube)
                    if label == 0:
                        filepath = output_path + "0/" + "subset" + str(subsetindex) + "/"
                        if not os.path.exists(filepath):
                            os.makedirs(filepath)
                        filename = str(fcount) + "_" + str(index)
                        np.save(filepath + filename + ".npy", node_cube)
...
if __name__ == '__main__':
    csv_path = "./data/luna16"
    get_node_classify(csv_path)